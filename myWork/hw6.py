...
def string3(str):
    ...
    new_str = str[-2:] *3 # -2 index means last two chars
    return new_str
...
def exchange(str):
    ...
    have been exchanged.
    """
    if len(str) == 1: # if string is only 1 letter, just return it
        return str
    else:
        #temporary variables for first and last chars
        temp_first = str[0] 
        temp_last = str[-1]
    
        #swap first and last chars when concatenating new string
        new_str = temp_last + str[1:-1] + temp_first
...
def count_list(lst):
    ...
    # creates list of 0s, length is one higher than max value in original list (to account for 0s)
    new_lst = [0] * (max(lst)+1)
    

    # new list index values incremented when they are found in original list
    for i in lst:
        new_lst[i] +=1
...
def remove_range(alist, min, max):
    ...
    *** Important: your code must use comprehensions and should not be more than
    ...
    """
    ''' function without comprehension
    new_alist = []

    for val in alist:
        if val < min or val > max:
            new_alist.append(val)
    print(new_alist)
    return new_alist
    '''
  

    #creates new list with values less than the min or greater than the max from original list
    new_alist = [val for val in alist if val < min or val > max]
...
def word_count_in_set(words):
    ...
    *** Solution should not be more than 3 lines of code (can be less)
    ...
    #converts words to a set, automatically erases duplicates
...
def topping1(dict):
    ...
    # if ice cream key found, topping changes to cherry
    for key in dict:
        if key == 'ice cream':
            dict[key] = 'cherry'
    ...
    # ** operator allows us to add existing dictionaries to a new dictionary
    new_dict = {** new_el,** dict} 
...
def friend_list(friend_dictionary):
    ...
    # navigate through original dictionary
    for person, friend in friend_dictionary.items():
        # if person not in new dictionary, add them as a key and add the friend to their friend list
        if person not in friend_map:
            friend_map.update({person:[friend]})
        # if person already in new dictionary, just add the friend to their friend list
        elif person in friend_map:
            friend_map[person].append(friend)
        
        # if friend not in new dictionary, add them as a key and add the person to their friend list
        if friend not in friend_map:
            friend_map.update({friend:[person]})
        # if friend already in new dictionary, just add the person to their friend list
        elif friend in friend_map:
            friend_map[friend].append(person)
